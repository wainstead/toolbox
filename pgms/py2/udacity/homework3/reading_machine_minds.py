...
def is_symbol(grammar, candidate):
    for rule in grammar:
        if candidate == rule[0]:
            return True
    return False

def is_terminal(grammar, token):
    return not is_symbol(grammar, token)

# returns: a list of terminals, i.e. ['c', 'd', 'b']
def cfgempty(grammar, symbol, visited):

    #print "received %s" % symbol
    #print "visited: %s" % visited

    if symbol in visited:
        #print "%s was visited, returning None" % symbol
        return None
    ...
    for rule in grammar:
        if symbol == rule[0]:
            #print "--> Doing rule %s for symbol %s" % (rule, symbol)
            exp = rule[1]
            mylist = list()
            if exp == []:
                # Epsilon
            ...
            for token in exp:
                #print "doing token %s in exp %s" % (token, exp)
                result = cfgempty(grammar, token, visited + [symbol])
                if result == []:
                    #print "Epsilon"
                    pass
    ...
    if not all(mylist):
        #print "Returning None for: %s" % mylist
        return None
    #print "returning %s" % mylist
    return mylist
